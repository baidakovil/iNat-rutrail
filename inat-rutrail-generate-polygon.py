...
track_file_path = 'kml_rutrail/Nerskaya.kml'
...
def create_buffer_track(
    coordinates: List[Tuple], buffer: float, buffer_clean: float
) -> List[Tuple]:
    """
...
def save_kml(empty_file_path, insert_string, track_file_path, buffer, buffer_clean):
    new_file_path = os.path.basename(track_file_path)[:-4] + '_buffered.kml'
    placemark_name = f'Autogenerated from file {os.path.basename(track_file_path)}\n'
...
def main(track_file_path: str, buffer: float, empty_file_path: str) -> None:
    ...
    buffer_clean_merc = meters_to_mercator_units(buffer_clean, coordinates_latlon)

    # Convert the coordinates from latitude and longitude to Mercator coordinates
    coord_merc = latlon_to(coordinates_latlon)
    ...
    solution_merc = create_buffer_track(coord_merc, buffer_merc, buffer_clean)
